...
<body style="margin:0; padding:24px; background:#f8fafc; font-family:Arial, sans-serif; color:#0f172a;">
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:700px; margin:0 auto; background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; overflow:hidden;">
        <tr>
            <td style="padding:18px 22px; background:#0f172a; color:#fff;">
                ...
                <h1 style="margin:0; font-size:21px;">{{ $isPaid ? 'Pedido pagado' : 'Nuevo pedido recibido' }}</h1>
                <p style="margin:6px 0 0; font-size:14px; color:#cbd5e1;">{{ $order->order_number }}</p>
            </td>
        </tr>
        <tr>
            <td style="padding:22px;">
                <p style="margin:0 0 12px; color:#334155;">Notificación enviada a {{ $storeEmail }}.</p>

                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse; margin-bottom:12px;">
                    <tr>
                        <td style="border:1px solid #e2e8f0; padding:8px;"><strong>Cliente:</strong> {{ $order->customer_full_name }}</td>
                        <td style="border:1px solid #e2e8f0; padding:8px;"><strong>Email:</strong> {{ $order->customer_email }}</td>
                    </tr>
                    <tr>
